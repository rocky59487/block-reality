...
public record AnalysisResult(
    ...
    public com.blockreality.api.AnalysisResult snapshot() {
        return new com.blockreality.api.AnalysisResult(revision, ok, singular, diagnostic, maxDc,
                governing, governingKind, islands, singularIslands, equilibriumResidual,
                bucklingFactor, bucklingState, members.stream().map(MemberSnapshot::snapshot).toList(),
                shells.stream().map(ShellSnapshot::snapshot).toList(), unassigned,
                overCapacity, bucklingCritical, bucklingIslands);
    }
}

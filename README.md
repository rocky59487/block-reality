...
![Verification](https://img.shields.io/badge/verification-330_engine_%2B_361_Java_checks_registered-blue)
...
| Java | 361 tests registered (299 core, 62 Forge-side); 28 of them start `br-sidecar` and run FrameCore for real, and 11 load the v0.4 engine library in-process |
...
| Java | 361 項測試已登錄（core 299、Forge 側 62），其中 28 項會實際啟動 `br-sidecar` 執行 FrameCore，另有 11 項在同一個程序內載入 v0.4 的引擎函式庫 |
...
Java 總數包含環境 SKIP；最新 Windows core299=287PASS/12SKIP、Forge62PASS，真原生JNA11/11。梁殼顯示與 channel7 使用不可變引擎樣本；原生梁目前每位置只輸出選定的一側，雙側回收未完成，詳見 docs/GATES.md。BSI 遊戲入口仍未換裝。

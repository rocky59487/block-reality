...
![Verification](https://img.shields.io/badge/verification-330_engine_%2B_407_Java_checks_registered-blue)
...
| Java | 407 tests registered (327 core, 80 Forge-side); 28 of them start `br-sidecar` and run FrameCore for real, and 13 load the v0.4 engine library in-process |
...
| Java | 407 項測試已登錄（core 327、Forge 側 80），其中 28 項會實際啟動 `br-sidecar` 執行 FrameCore，另有 13 項在同一個程序內載入 v0.4 的引擎函式庫 |
...
Java 總數包含環境 SKIP；最新 Windows core305=293PASS/12SKIP、Forge64PASS，真原生JNA12/12。梁殼顯示與 channel8 使用不可變引擎樣本；BSI stationIdentity 可選雙側身份已接到真JNA/顯示與封包，舊請求維持單側，詳見 docs/GATES.md。BSI 遊戲入口仍未換裝。

Native engine packaging for the current development branch is documented in [NATIVE_PACKAGING](docs/NATIVE_PACKAGING.md). The Windows/Linux jar has been validated with real v1.3 libraries; Minecraft game-loop migration remains pending.

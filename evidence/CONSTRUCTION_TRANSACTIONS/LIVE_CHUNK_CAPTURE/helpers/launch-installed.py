...
root=Path('/home/rocky/br-live-chunk-capture');server=root/'installed-server';out=root/'qualification/runtime-first'
assert server.resolve().is_relative_to(root.resolve()) and (server/'OWNER').read_text().strip()=='block-reality-live-capture-runtime-v1'
...
with (out/'server-first.log').open('xb') as log:
    ...
    print('Owned live-capture server PID',process.pid,flush=True)

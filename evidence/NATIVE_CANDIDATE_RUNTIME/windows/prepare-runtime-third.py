...
out=root/'qualification/runtime-third-setup.json';assert not out.exists()
config=root/'forge/run/runtime-smoke/serverconfig/blockreality-server.toml'
text=config.read_text();text,n=re.subn(r'(?m)^(\s*mode\s*=\s*)"(?:INPROCESS|OFF)"',r'\1"OFF"',text);assert n==1

...
MUTATIONS = {
    "FLATTEN": ('        require(STATES.get(aggregate).equals(stateToken),',
                '        if (islandResults.stream().anyMatch(v -> v.state().hasFactor())) aggregate = 0;\n        require(STATES.get(aggregate).equals(stateToken),'),
    "FACTOR_LEAK": ('        if (!state.hasFactor()) factor = 0;', ''),
    "CLEAR_CRITICAL": ('        var unassigned = unassigned(h);',
                       '        if (!state.hasFactor()) critical = false;\n        var unassigned = unassigned(h);'),
    "DROP_ISLANDS": ('unassigned, overloaded, critical, islandResults);',
                     'unassigned, overloaded, critical, List.of());'),
}

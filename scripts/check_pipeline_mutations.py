#!/usr/bin/env python3
"""MP-6: compile two behavioral fault arms in a disposable source copy, leaving the checkout intact."""
import argparse
import json
import os
...
def main():
    ...
    arms = [
        ('stopped-span', 'mod', 'core/src/main/java/com/blockreality/core/diagnostics/PipelineProfile.java',
         'if (!session.accepting) return;', 'if (false) return;',
         ':core:test', '*PipelineProfileTest.stopAndRestartRejectOldWorkAndPreserveImmutableStoppedSnapshot',
         'core/build/test-results/test', 'stopAndRestartRejectOldWorkAndPreserveImmutableStoppedSnapshot()')]
    results = []
    (root / 'build').mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='mp-mutants-', dir=root / 'build') as temporary:
        work = Path(temporary)

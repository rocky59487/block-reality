#!/usr/bin/env python3
"""RS-3: compile snapshot ownership fault arms in a disposable source copy, leaving the checkout intact."""
import argparse
import json
import os
...
def main():
    ...
    arms = [
        ('aliased-capture', 'mod', 'core/src/main/java/com/blockreality/core/world/ConstructionLedger.java',
         'var captured = new HashMap<>(cells);', 'var captured = cells;',
         ':core:test', '*ConstructionSnapshotTest.workSnapshotDoesNotAliasCallerCollections',
         'core/build/test-results/test', 'workSnapshotDoesNotAliasCallerCollections()'),
        ('mutable-entry', 'mod', 'core/src/main/java/com/blockreality/core/world/ConstructionLedger.java',
         'return Map.entry(entry.getKey(), entry.getValue());', 'return entry;',
         ':core:test', '*ConstructionSnapshotTest.workAndGraphViewsRejectEveryMutableEntryPoint',
         'core/build/test-results/test', 'workAndGraphViewsRejectEveryMutableEntryPoint()'),

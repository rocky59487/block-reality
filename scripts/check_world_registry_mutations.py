#!/usr/bin/env python3
"""WR-10: compile two behavioral fault arms in a disposable source copy, leaving the checkout intact."""
import argparse
import json
import os
...
def main():
    ...
    arms = [
        ('checksum', 'mod', 'core/src/main/java/com/blockreality/core/world/WorldCellIndex.java',
         'if (!MessageDigest.isEqual(digest(bytes, bytes.length - 32),',
         'if (false && !MessageDigest.isEqual(digest(bytes, bytes.length - 32),',
         ':core:test', '*WorldCellIndexTest.damagedChecksumsAndEveryTruncationFailClosed',
         'core/build/test-results/test', 'damagedChecksumsAndEveryTruncationFailClosed()'),
        ('missing-input', 'forge', 'src/main/java/com/blockreality/impl/server/InputCoverage.java',
         'if (!complete()) return false;', 'if (false) return false;',
         'test', '*InputCoverageTest.unreadableStructureAndGroundNeverDispatch',
         'build/test-results/test', 'unreadableStructureAndGroundNeverDispatch()')]
    results = []
    (root / 'build').mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='wr-mutants-', dir=root / 'build') as temporary:
        work = Path(temporary)

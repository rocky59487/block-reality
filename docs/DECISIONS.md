...
2026-09-10 最新責任與 v1 交付：見 D-047、`V1_MODULE_PROGRAM.md`。
施工 piece 邊界、交易日誌與完整驗收：見 D-048、`CONSTRUCTION_TRANSACTIONS.md`。

**今天的骨架**（要知道這個專案現在長什麼樣，讀這十條就夠）：
...
| **D-010** | 結構模型來自「骨」不是「肉」 |
| **D-046** | 引擎 v2 程序的消費者對位：**等的是 tectonic2 #20，不等其拆 `Session`**；契約加法批次 #2 綁 v0.5 倒塌，v0.4 判準不動 |
...
D-043 ↔ D2-010（BSI v1）、D-044 ↔ D2-013（出貨形狀）、D-045 ↔ D2-015（對數臂）、D-046 ↔ D2-016（v2 程序）。
逐項對位在 `docs/ALIGNMENT_LEDGER.md`（兩倉逐位鏡像）。

---

## D-048 · 施工 piece 的誕生與恢復由原子交易負責

**決定**（2026-09-10，依使用者自主整合架構授權）：確認新施工時，每個明示的製造
piece 取得新的伺服器 UUID；相鄰放置不默默延長/融合既有 piece。觀測式分組仍能描述
材料聚合區域，其既有 ID 與歷史保留，但不能授權消耗、退款或 inverse undo。
接頭、材料/斷面切換、轉角與明示邊界拆成獨立 piece；未來延長需另有明確交易規則。

世界、玩家庫存與模組檔案是不同持久化參與者。先保存 PREPARED before/after intent，
全部套用並確實 flush 後才保存 COMMITTED，再公布 revision/原生請求與回覆。未提交
故障回滾；回滾/決策驗證失敗保留日誌並停止後續變更，恢復不得覆寫未知外部變更。
重送綁 actor/domain/base revision/內容，退役 IDs 永不重用；不得只靠記憶體去重。

**理由**：#12 的材料、方塊、身分與單次 revision 必須一起成立；目前放置後的 metadata
分組沒有此保證。顯式 piece 邊界也避免把損傷/製造歷史隨相鄰放置悄悄改寫。
**否證條件**：任一真 Forge/重啟故障留下半套狀態、重送產出新材料/ID、或界面把觀測
分組當成交易權威，則 CT 不得通過。無法在凍定規模維持 v1 效能時須改持久化策略，
不能移除去重/回滾/持久性。完整門檻見 `CONSTRUCTION_TRANSACTIONS.md`。

## D-047 · v1 只做模組，所有物理與動態姿態由引擎交付

**決定**（2026-09-10，使用者明示）：本工作只改 block-reality 模組，不改引擎倉。
Java 不做後處理、不手寫物理；剛體滾動/壓碎/倒塌需引擎事件與姿態。
因此 D-031/D-032 與 V04_PLAN 中的自家簡化剛體/重力 fallback 不再是 v1 可用的實作方案。
粒子與樣本視覺插值仍可呈現引擎已決定的結果，不能決定新的失效或運動。
中途版可以發布，但未交付能力明示不提供，不能據此宣稱 v1 完成。

**理由**：維持單一物理權威並避免與正在進行的引擎工作衝突。
**否證條件**：使用者另行改變責任/產品範圍時重裁；引擎缺動態介面本身不是放寬 Java 邊界的理由。
具體分工、issues、依賴鏈及驗收见 `V1_MODULE_PROGRAM.md`。

## D-046 · 引擎 v2 程序的消費者對位：本倉等的是 tectonic2 #20，不是它拆 `Session`；契約加法批次 #2 是 v0.5 倒塌的前提

**決定**（2026-09-04；operator：「確保現在兩專案真的整合了……把 tectonic2 跟他的所有做完跟沒做完的 MC 功能跟引擎功能融合好……開始把 perfect engine 做好……先做好 plan」）：

1. **整合現況以量到的為準**：介面層已整合（契約逐位、pin 自洽、N24-b5 首次在 CI 之外對本機建的引擎跑綠）、能力層未整合（引擎 `capabilities:[]`）、
   產品層未切換（#89）。數字在 `docs/GATES.md` 2026-09-04 與 tectonic2 `docs/V2_PROGRAM.md` §1；本條不重複。
2. **本倉不等引擎的 v2 重構。** tectonic2 D2-016 把消費者對位（MC63 / MC64 / MC65a / MC65b / MC60d、`bsi.core` 宣告）放在**只等 V2_PIN** 的軌 A；
   #89 的引擎側前提隨 tectonic2 #20 成立，與其 `Session` 拆分（V2_STATE / V2_LANES）**無關**。本倉在 #20 關閉後即接線，不等其 v1.6。
3. **契約加法批次 #2 是 v0.5 倒塌的前提，不是 v0.4 的。** `include:"islands"`（逐島生命週期／旗標／逐族有效性）與 `bsi.fracture.step`（`broken` / `fragments`）
   在 tectonic2 v2.0-alpha 隨其 `BSI_ADD2.md` 先凍再落地；本倉 v0.4 的驗收（N19–N24）**一條不因它移動**。落地時走 BSI_ADD1 的同一套機械面：
   兩倉 `contract/` 逐位、`.github/tectonic2-contract-ref` 兩行、`diff -r` 空、`BsiRecords` 加記錄型別、腿走完位元組合。
4. **v0.5 的倒塌吃引擎的碎塊，不自己算。** 脫離集合、質量、質心、慣性張量、初速全部來自 `fragments` 區段（引擎 MC25–MC28 的同一份算術）；
   `FallingBlockEntity` 群組只演出（「算的歸算，演的歸演」）；display 軌永不觸發 handoff（#16 的既有裁決，契約以 `UNSUPPORTED` 執行）。
5. **引擎版號只顯示不裁決。** `bsi.hello` 的 `version` 從 1.2.0 走到 2.0.0 對本倉零意義；相容性只看 `contractSha256` 與 `bsi_capi` 的 ABI 版本（D-044 第 3 條）。

**理由**：把「引擎在重構」讀成「消費者要等」是最容易發生、也最貴的誤讀——tectonic2 的軌 A 明文只等釘住不等拆分，本倉的阻擋項自始至終是 #20（MC65a + MC64）。
批次 #2 綁 v0.5 而不是 v0.4，是因為 v0.4 的判準已凍（N19–N24），把新區段塞進去等於事後移線（鐵則 1）。

**代價（照登）**：v0.5 倒塌的判定側**完全依賴**引擎的 PE-4；在它落地前，本倉的倒塌只能是 v0.4 既有的「D/C ≥ 1 標紅」，沒有脫離集合。
`docs/V04_PLAN.md` §5 的「幾乎已在 wire 上」指的是 tectonic2 的 **v2 wire**（`fracture.step`），**不是 BSI**——這句話在 BSI 上今天不成立，直到 `bsi.fracture` 有動詞（§5 已 dated 追記）。

**否證條件**：
(1) tectonic2 #20 關閉後 #89 接線仍失敗且原因在引擎的 `Session` 拆分 → 本條第 2 點錯，改為等其 v1.6，dated 記於此；
(2) v0.5 的演出需要 display 軌的斷裂**預覽** → 提契約 `bsi.fracture.preview`（唯讀），不放寬 P9，不在本條下私自用 display 軌觸發；
(3) 批次 #2 的任何區段需要 Java 端重算力學（例如自己算慣性張量）→ 違反 D-033，該區段回引擎補欄位；
(4) 本條被引用為「v0.4 可以等批次 #2 一起出」→ 誤用：v0.4 的出口是 N19–N24，與批次 #2 無關。

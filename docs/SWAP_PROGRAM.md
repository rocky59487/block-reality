...
- **2026-09-04（引擎 v2 程序 + D-046；原文不改）**：
  1. **整合稽核量到**：介面層已整合（契約逐位、N24-b5 首次在 CI 之外對本機建的引擎跑綠、`verify.py` 330）、能力層未整合（`capabilities:[]`）、產品層未切換（#89）。
     細節 tectonic2 `docs/V2_PROGRAM.md` §1、本倉 `docs/GATES.md` 2026-09-04。
  2. **引擎側 v2 程序**（tectonic2 D2-016）：設計鎖 `docs/specs/V2_ARCHITECTURE.md`、計畫 `docs/V2_PROGRAM.md`。
     §6 的版本綁定表改讀為其 §4：**MC68 能力宣告隨 #20 進 v1.4**（不是 v1.6）；v1.6 = 引擎骨架零行為 + 遙測；v2.0-alpha = 契約加法批次 #2。
  3. **本倉的對位 D-046**：**只等 #20，不等引擎拆 `Session`**；Phase 3（#89）的引擎側前提隨 #20 成立。
     批次 #2（`include:"islands"`、`bsi.fracture.step`）綁 **v0.5 倒塌**，v0.4 的 N19–N24 一條不動。
  4. §6 Phase 5 的「語料對真引擎 exit 0」今天量到的是**全 SKIP 的 exit 0**（誠實狀態，不是綠）；真正的綠要等帳清空後不帶 `--assume-caps` 執行 C-4 / C-5 / C-7 / C-11。

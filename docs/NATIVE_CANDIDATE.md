...
> 此文件保存模組 #119（bbe9ccd）的交付紀錄與當時測試範圍。其 SidecarClient/待接線
> 敘述只適用該較早分支；目前 GAME_RUNTIME/HUD/存檔整合與新庫驗收另見
> [NATIVE_CANDIDATE_RUNTIME](NATIVE_CANDIDATE_RUNTIME.md)，不將舊測試數冒充最新結果。

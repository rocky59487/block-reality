...
**實測開與關的數字到最後一位都相同**，117 項 gate 兩邊都全過。
...
最後一行要是 `ALL PASS`（117 項）。**這一步失敗就不要往下走**——後面看到的任何東西都不可信。
...
104 個測試，0 failures。其中 25 個會**實際啟動 `br-sidecar` 子程序**跑真的 FrameCore。

沒帶 `-Dbr.sidecar` 那 25 個會 skip 而不是假裝通過。**帶了但路徑不對則直接建置失敗**，
不會 skip——先前那一行相對路徑在 forked test JVM 裡解析不到，25 個測試全部靜靜 skip 而
...
| | |
|---|---|
| 對閉合解（非零參考） | **31 項，最差相對誤差 1.623e-10**，RMS 3.165e-11 |
| 對恰為零的參考 | 10 項，最差絕對殘差 **1.49e-08 N·mm** |
| 無閉合解的性質 | **10/10**（機構偵測、單方塊、荷載拒絕、缺料拒絕、未知斷面拒絕、重複求解逐位元相同、島隔離、板不是格梁、實心板拒絕、支承還原不會降低需求） |

...
|---:|---:|---:|---:|
| 1 | 12 | 0.51 ms | 0.46 ms |
| 19 | 120 | 8.5 ms | 7.3 ms |
| 99 | 600 | 40.7 ms | 37.2 ms |
| 199 | 1200 | 117 ms | 100 ms |

**199 根構件約 0.1 s**——而且求解不在 tick 執行緒上，所以這是「拿到結果的延遲」
而不是「從遊戲拿走的時間」。傳輸本身的成本另有專節：同一個 86 構件模型，
共用記憶體 4.5 ms、JSON 28.3 ms（省 84%）。

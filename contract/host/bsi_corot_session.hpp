// Included in SessionImpl: all transports and pending replies share this path.
    void corotRequest(const Request& rq,const uint8_t* payload,size_t n,Reply& out) {
        ...
        const bsi_corot_view* view=nullptr;nativeFault_=true;const int status=engine_.vt->corot_solve(inst_,&input.input,&view,&writer);
        if(status!=BSI_OK){nativeFault_=false;errorFromBuilder(out,rq,status,builder);return;}
        if(builder.hasError()||!corot_wire::analysis(view,input.input.options,artifactNamespace_,world_,owners_,vocab_,pack)){wireError(rq,out,"INTERNAL","invalid finite analysis delivery; reopen session");return;}
